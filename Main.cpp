...
#define SETTINGS_NUMBER_OF_CHECKBOXES 30.f //This scales both mgr and settings windows
	...
	bool save_res = SaveStartupInfo(startup_info, info_path.known_folder + L"\\" + info_path.info_folder, info_path.info_file + L"." + info_path.info_extension);
	if (!save_res) ShowLastError(L"Failed to save application data", RS(SCV_LANG_ERROR_SMARTVEIL));
